...
// DYNAMIC CATEGORY SELECTOR (same as in Add.jsx)
function DynamicCategorySelector({ onSelect, value }) {
  ...
  useEffect(() => {
    if (value) {
      (async () => {
        // If value is an object, use its _id, else value as string
        let valueId =
          typeof value === "object"
            ? value._id
              ? value._id
...
const EditProductPopup = ({ product, token, onClose, onUpdate }) => {
  // Patch: always use id string for category
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg w-full max-w-2xl overflow-y-auto max-h-[90vh] relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Description" className="border px-3 py-2" />
          {/* Price */}
          <input type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="Price" className="border px-3 py-2 w-32" />
          {/* Category selector (dynamic, tree-based) */}
          <div>
